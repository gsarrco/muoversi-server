...
from MuoVErsi.sources.base import Route, Source
...
class StopTimesFilter:
    ...
    def get_times(self, db_file: Source, service_ids) -> tuple[list[Route], tuple]:
    ...
    def format_times_text(self, results: list[Route], _, lang):
        ...
        for result in results:
            line, headsign, trip_id, stop_sequence = result.route_name, result.headsign, \
                                                                  result.trip_id, result.dep_stop_time.stop_sequence

            time_format = result.dep_stop_time.dt.strftime('%H:%M')

            if result.dep_stop_time.delay > 0:
                time_format += f'+{result.dep_stop_time.delay}m'

            if result.arr_stop_time:
                arr_time = result.arr_stop_time.dt.strftime('%H:%M')
                time_format += f'->{arr_time}'

                if result.arr_stop_time.delay > 0:
                    time_format += f'+{result.arr_stop_time.delay}m'

            if result.dep_stop_time.platform:
                line = f'<b>{time_format}</b> {headsign}\n⎿ {line} BIN. {result.dep_stop_time.platform}'
            else:
                line = f'<b>{time_format}</b> {line} {headsign}'

            if result.dep_stop_time.dt < datetime.now():
                line = f'<del>{line}</del>'
