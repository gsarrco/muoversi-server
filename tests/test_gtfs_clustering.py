...
def test_data() -> list[tuple[str, str, str]]:
    return [
        ...
        ('VENEZIA CORSIA C', 'VENEZIA', 'CORSIA C'),
...
def test_get_root_from_stop_name(test_data):
    for test_datum in test_data:
        assert get_root_from_stop_name(test_datum[0]) == test_datum[1]


def test_get_loc_from_stop_and_cluster(test_data):
    for test_datum in test_data:
        assert get_loc_from_stop_and_cluster(test_datum[0], test_datum[1]) == test_datum[2]
